...
"""COMPROMISE (UPDATED): Shallow multimodal model with all context modules.

Key design choices (unchanged):
1. One hidden layer per module
2. Direct outcome prediction (W/D/L) alongside rates
3. Simple Poisson loss
4. Linear fusion

New: Now includes league, season, and matchup embeddings.
"""
...
def combined_loss(lam_h, lam_a, outcome_logits, hg, ag, outcome, outcome_weight=0.5):
    poisson_loss = F.poisson_nll_loss(lam_h, hg, log_input=False, full=True) + \
                   F.poisson_nll_loss(lam_a, ag, log_input=False, full=True)
    outcome_loss = F.cross_entropy(outcome_logits, outcome)
...
# ============================================================================
# TRAINING (unchanged, just more modules)
# ============================================================================
...
def train_module(module_name, config, device):
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            lam_h, lam_a, outcome_logits = model(x)
            loss = combined_loss(lam_h, lam_a, outcome_logits, hg, ag, outcome, config.outcome_weight)
            loss.backward()
            optimizer.step()
...
def train_fusion(config, device):
    """Simple fusion on module predictions (now with 7 modules)."""
    print("\n" + "=" * 70)
    print("TRAINING FUSION (Shallow, 7 modules)")
    print("=" * 70)
    ...
    # Fusion input dim: 7 modules × 3 features = 21
    ...
    for epoch in range(30):
        ...
        val_m = compute_metrics(model, fusion_val_loader, device)
        test_m = compute_metrics(model, fusion_test_loader, device)
    ...
    torch.save({"state_dict": model.state_dict(), "best_accuracy": best_acc}, COMPROMISE_MODEL_PATH)
    
    final_metrics = compute_metrics(model, fusion_test_loader, device)
...
def main():
    ...
    print(f"Validation split: {config.val_fraction:.0%}")
