...
def scrape_league(league_name: str, league_code: str) -> list:
    ...
    for match in matches:
        ...
        # Build dictionary with the new Competition column
        current_match = {
...
def main():
    ...
    df_lineups = pd.DataFrame(all_matches)
    ...
    if not df_lineups.empty:
        ...
        # Fixed reference to df_lineups.columns
        player_cols = [c for c in df_lineups.columns if c not in base_cols]
        df_lineups = df_lineups[base_cols + player_cols]
    ...
    print(f"\n--- Scraping Complete ---")
    if not df_lineups.empty:
        print(f"Total matches scraped: {len(df_lineups)}")
